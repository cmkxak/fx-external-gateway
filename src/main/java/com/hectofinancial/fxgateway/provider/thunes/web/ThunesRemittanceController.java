...
public class ThunesRemittanceController {
    ...
    public QuotationResponse createQuotation(@Valid @RequestBody QuotationRequest req) {
        return provider.createQuotation(req);
    }

    /** ③ 수취인 검증. type = C2C | C2B | B2C | B2B */
    @PostMapping("/payers/{payerId}/{type}/credit-party-verification")
    public String verifyBeneficiary(@PathVariable String payerId,
                                    @PathVariable String type,
                                    @Valid @RequestBody VerificationRequest req) {
        return provider.verifyBeneficiary(payerId, type, req);
    }

    /** ④ 거래 생성 (견적 ID 기준) */
    @PostMapping("/quotations/{quotationId}/transactions")
    public TransactionResponse createTransaction(@PathVariable long quotationId,
                                                 @Valid @RequestBody TransactionRequest req) {
        return provider.createTransaction(quotationId, req);
    }

    /** [선택] 거래 확정 — 자금 이동 트리거 */
    @PostMapping("/transactions/{transactionId}/confirm")
    public TransactionResponse confirmTransaction(@PathVariable long transactionId) {
        return provider.confirmTransaction(transactionId);
    }
}
